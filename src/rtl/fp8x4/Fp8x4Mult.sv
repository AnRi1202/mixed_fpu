...
//
// Area-sharing policy (mirrors the Fp8x4Add philosophy of "share the big block,
// cut/route per lane"):
//
//   * Significand product:
//       Unlike addition, a single `*` cannot be lane-split into 4 independent
//       SIMD products: with A = a3*2^3G + a2*2^2G + a1*2^G + a0, the inner
//       diagonal a1*b1 collides (same bit position) with the cross terms
//       a2*b0 + a0*b2, so gaps cannot isolate it. (Isolating 4 lanes in one
//       multiplier would need a ~60x60 array, ~6x the 24x24 -> not worth it.)
//
//       Therefore ONE 25x25 multiplier is shared by:
//         - FMT_FP32                                 (full 25x25)
//         - FMT_BF16  : 2 packed 8x8 (gap trick)     (lanes hi, lo)
//         - FMT_FP8   : 2 packed 4x4 (gap trick)     (lanes 3 and 1)
//       and only TWO extra tiny 4x4 multipliers are added for FMT_FP8 lanes 2,0.
//       Extra hardware vs. the BF16/FP32-only design is just 2 x (4x4).
//
//   * Exponent path, the 31-bit rounding adder and result packing are shared
//     across all three formats, exactly like Fp8x4Add.
//
//   (A fully reconfigurable Zhan-2019-style CSA nibble array could fold the FP8
//    lanes into the big multiplier for a bit more sharing, at much higher RTL
//    complexity; left as a future optimization.)
// =============================================================================
module Fp8x4Mult(
    input  logic       clk,
    ...
    // =========================================================================
    // Shared 25x25 significand multiplier
    // -------------------------------------------------------------------------
    // FMT_FP32 : {2'b01, frac23}                        -> full product
    // FMT_BF16 : {1,hi7} , 9'b0 , {1,lo7}               -> hi at [49:34], lo at [15:0]
    // FMT_FP8  : 9'b0 , {1,lane3} , 8'b0 , {1,lane1}    -> lane3 at [31:24], lane1 at [7:0]
    //
    // FP8 packing check (operands 4-bit, lane3 sig @ bit12, lane1 sig @ bit0):
    //   P1 = sig1*sig1 (<=225)              -> bits [7:0]
    //   cross = sig3*sig1 + sig1*sig3 (<512) at 2^12 -> bits [20:12]
    //   P3 = sig3*sig3 (<=225) at 2^24      -> bits [31:24]   (gap [23:21] clean)
    // =========================================================================
    ...
    always_comb begin
        case (fmt)
            FMT_FP32: begin
                sigX = {2'b01, mx.fp32.frac};
                sigY = {2'b01, my.fp32.frac};
            end
            FMT_BF16: begin
                sigX = {1'b1, mx.bf16x2.hi[6:0], 9'b0, 1'b1, mx.bf16x2.lo[6:0]};
                sigY = {1'b1, my.bf16x2.hi[6:0], 9'b0, 1'b1, my.bf16x2.lo[6:0]};
            end
            default: begin // FMT_FP8 : pack lanes 3 and 1
                sigX = {9'b0, 1'b1, mx.fp8x4.lane3[2:0], 8'b0, 1'b1, mx.fp8x4.lane1[2:0]};
                sigY = {9'b0, 1'b1, my.fp8x4.lane3[2:0], 8'b0, 1'b1, my.fp8x4.lane1[2:0]};
            end
        endcase
    end
    assign sigProd = sigX * sigY;

    // Two extra tiny 4x4 multipliers for FP8 lanes 2 and 0
    logic [3:0] sigX2, sigY2, sigX0, sigY0;
    logic [7:0] prod2, prod0;
    assign sigX2 = {1'b1, mx.fp8x4.lane2[2:0]};
    assign sigY2 = {1'b1, my.fp8x4.lane2[2:0]};
    assign sigX0 = {1'b1, mx.fp8x4.lane0[2:0]};
    assign sigY0 = {1'b1, my.fp8x4.lane0[2:0]};
    assign prod2 = sigX2 * sigY2;
    assign prod0 = sigX0 * sigY0;
    ...
    // =========================================================================
    // FP8 per-lane normalization + frac/round
    // -------------------------------------------------------------------------
    ...
    always_comb begin
        prod_f8[0] = prod0;
        prod_f8[1] = sigProd[7:0];
        prod_f8[2] = prod2;
        prod_f8[3] = sigProd[31:24];
